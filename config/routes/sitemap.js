...
module.exports.default = function (req, res) {
  HelperService.getPageData().then(data => {
    ...
    function findRouteModels(route) {
      if(route.sitemapModel) routeModels.push(route.sitemapModel);
    ...
    function generateAndSend() {
      ...
      function generateXml(baseUrl, route) {
        if(!route.sitemapModel) { // if there is static url
          ...
          var applyConfig = {[route.path]: JSON.parse(JSON.stringify(routeModelItems[route.sitemapModel.name])) };
